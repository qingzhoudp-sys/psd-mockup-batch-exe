...
(function () {
    ...
    function autoLayoutText(layer,value,doc) {
        ...
        // Write contents first; optional layout uses only a layer transform.
        var item=layer.textItem;
        var original="";
        try { original=String(item.contents).replace(/[\r\n]/g,""); } catch(ignore) {}
        var capacity=Math.max(4,original.length);
        try { item.contents=value; }
        catch(domTextError) { setTextContentsByAction(value); }
        if(original.length && value.length>capacity) {
            try {
                var lines=Math.min(2,Math.ceil(value.length/capacity));
                var lineLimit=Math.ceil(value.length/lines);
                item.contents=wrapFileName(value,lineLimit);
                var factor=Math.max(0.35,Math.min(1,capacity/lineLimit,1/lines));
                layer.resize(factor*100,factor*100,AnchorPosition.MIDDLECENTER);
            } catch(layoutIgnore) {}
        }
    }
