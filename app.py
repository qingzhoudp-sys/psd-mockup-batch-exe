...
(function () {
    ...
    try {
        ...
        return JSON.stringify({ok:true,total:files.length,success:successes,failures:failures,exported:exported});
    } catch(e) {
        return JSON.stringify({ok:false,error:e.message,success:successes,failures:failures,exported:exported});
