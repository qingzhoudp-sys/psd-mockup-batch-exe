...
(function () {
    ...
    function autoLayoutText(layer,value,doc) {
        // Photoshop 2020 can throw a generic error when repeatedly querying
        // text bounds after changing contents. Use the template text length as
        // a stable capacity reference and calculate layout only once.
        var item=layer.textItem;
        var original=String(item.contents).replace(/[\r\n]/g,"");
        var capacity=Math.max(4,original.length);
        var originalSize=parseFloat(item.size);
        if(!originalSize || originalSize<=0) originalSize=12;
        var lines=Math.max(1,Math.ceil(value.length/capacity));
        var lineLimit=Math.max(capacity,Math.ceil(value.length/Math.min(lines,2)));
        item.contents=wrapFileName(value,lineLimit);
        if(value.length>capacity) {
            var widthFactor=capacity/Math.min(value.length,lineLimit);
            var heightFactor=1/Math.min(lines,2);
            var newSize=Math.max(5,originalSize*Math.min(1,widthFactor,heightFactor));
            item.size=UnitValue(newSize,"pt");
        }
    }
