...
(function () {
    ...
    function autoLayoutText(layer,value,doc) {
        ...
        // Write contents first, then cover-fit the editable text layer to the
        // complete Smart Object canvas without touching the font-size property.
        var item=layer.textItem;
        try { item.contents=value; }
        catch(domTextError) { setTextContentsByAction(value); }
        fitLayer(layer,doc.width.as("px"),doc.height.as("px"),"cover");
    }
