...
(function () {
    ...
    var successes = 0, failures = [], exported = [], reservedPaths = {};
    try {
    ...
    function processMain(materials,mockupFile,outputFolder) {
        ...
        try {
            ...
            var outputBase=uniqueOutputBase(outputFolder,base,true,false,false);
            exportWebJpg(doc,new File(outputFolder.fsName+"/"+outputBase+".jpg"));
    ...
    function processOne(artFile, mockupFile, outputFolder, fileIndex) {
        ...
        try {
            ...
            var outputBase=uniqueOutputBase(outputFolder,base,CFG.jpg,CFG.png,CFG.slices);
            if (CFG.jpg) exportJpg(doc, new File(outputFolder.fsName+"/"+outputBase+".jpg"), CFG.quality);
            if (CFG.png) exportPng(doc, new File(outputFolder.fsName+"/"+outputBase+".png"));
            if (CFG.slices) {
                try { exportSlicesJpg(doc, outputFolder, outputBase); }
    ...
    function outputNameTaken(folder,base,wantJpg,wantPng,wantSlices) {
        var root=folder.fsName+"/"+base;
        if(wantJpg) {
            var jpgPath=root+".jpg";
            if(new File(jpgPath).exists || reservedPaths[outputPathKey(jpgPath)]) return true;
        }
        if(wantPng) {
            var pngPath=root+".png";
            if(new File(pngPath).exists || reservedPaths[outputPathKey(pngPath)]) return true;
        }
        if(wantSlices) {
            var slicePath=root+"_切片";
            if(new Folder(slicePath).exists || reservedPaths[outputPathKey(slicePath)]) return true;
        }
        return false;
    }
    function reserveOutputName(folder,base,wantJpg,wantPng,wantSlices) {
        var root=folder.fsName+"/"+base;
        if(wantJpg) reservedPaths[outputPathKey(root+".jpg")]=true;
        if(wantPng) reservedPaths[outputPathKey(root+".png")]=true;
        if(wantSlices) reservedPaths[outputPathKey(root+"_切片")]=true;
    }
    function uniqueOutputBase(folder,base,wantJpg,wantPng,wantSlices) {
        var candidate=base,n=2;
        while(outputNameTaken(folder,candidate,wantJpg,wantPng,wantSlices)) {
            candidate=base+"_"+n; n++;
        }
        reserveOutputName(folder,candidate,wantJpg,wantPng,wantSlices);
        return candidate;
    }
...
class App(BaseTk):
    ...
    def _build_sku_page(self, root):
        ...
        ttk.Label(opts, text="防重名：发现同名图片时自动添加 _2、_3……；JPG、PNG 与切片使用相同编号，不覆盖旧文件。",
                  foreground="#075d32", wraplength=790).pack(anchor="w", pady=(10, 0))
    ...
    def _build_main_page(self, root):
        ...
        ttk.Label(opts, text="防重名：发现同名主图时自动添加 _2、_3……，保留原图片，不覆盖。",
                  foreground="#075d32").pack(anchor="w", pady=(8, 0))
...
def self_test():
    ...
    dedupe_checks = ["uniqueOutputBase", "outputNameTaken", "reserveOutputName", "reservedPaths",
                     'candidate=base+"_"+n', "new Folder(slicePath).exists"]
    missing_dedupe = [item for item in dedupe_checks if item not in script]
    if missing_dedupe:
        raise RuntimeError("自检失败，图片防重名缺少：" + ", ".join(missing_dedupe))
