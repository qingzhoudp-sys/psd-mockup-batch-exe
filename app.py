...
(function () {
    ...
    function replaceTextSmartObject(fileBase) {
        ...
        try {
            ...
            try { textDoc.close(SaveOptions.DONOTSAVECHANGES); } catch(ignore) {}
    ...
    function autoLayoutText(layer,value,doc) {
        var old=layer.bounds;
        var maxW=Math.max(old[2].as("px")-old[0].as("px"),doc.width.as("px")*0.35);
        var maxH=Math.max(old[3].as("px")-old[1].as("px"),doc.height.as("px")*0.18);
        var chars=Math.max(4,Math.floor(maxW/Math.max(8,layer.textItem.size.as("pt"))*1.7));
        layer.textItem.contents=wrapFileName(value,chars);
        var size=layer.textItem.size.as("pt"), minSize=5;
        for(var n=0;n<40;n++) {
            var b=layer.bounds,w=b[2].as("px")-b[0].as("px"),h=b[3].as("px")-b[1].as("px");
            if(w<=maxW && h<=maxH) break;
            size=Math.max(minSize,size*0.92); layer.textItem.size=UnitValue(size,"pt");
