...
(function () {
    ...
    try {
        ...
        if(CFG.mode=="main") {
            ...
            totalItems=processMain(files,mockup,outputFolder);
            successes=totalItems;
        } else {
    ...
    function processMain(materials,mockupFile,outputFolder) {
        var doc=app.open(mockupFile), stage="查找智能对象";
        try {
            var targets=[]; collectSmartObjects(doc,targets);
            if(!targets.length) throw new Error("样机中没有智能对象图层");
            var seen={}, materialIndex=0;
            for(var i=0;i<targets.length;i++) {
                var entry=targets[i], key="$"+entry.key;
                if(seen[key]) continue;
                seen[key]=true;
                var artFile=materials[materialIndex%materials.length];
                materialIndex++;
                stage="替换第 "+(i+1)+" 个智能对象“"+entry.layer.name+"”";
...
class App(BaseTk):
    ...
    def _build_main_page(self, root):
        self.main_mockup = self._path_row(root, "主图 PSD / PSB", self._choose_main_mockup)
    ...
    def _choose_main_mockup(self):
        p = filedialog.askopenfilename(title="选择主图样机", filetypes=[("Photoshop 样机", "*.psd *.psb"), ("所有文件", "*.*")])
        if p: self.main_mockup.set(p)
    ...
    def _validate_main(self):
        if not Path(self.main_mockup.get()).is_file(): return "请选择有效的主图 PSD/PSB 样机"
    ...
    def _start_main(self):
        ...
        settings = {
            "mode":"main", "mockup":str(Path(self.main_mockup.get()).resolve()),
        ...
        self._set_running(True); self.status.set("正在连接 Photoshop…")
        self._append("开始主图任务：" + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        threading.Thread(target=self._worker, args=(settings,), daemon=True).start()
        ...
        try:
            while True:
                ...
                if event[0] == "status": self.status.set(event[1])
                    ...
                    if r.get("ok"):
                        if s.get("mode") == "main":
                            lines=[f"主图完成：已更新智能对象 {r['success']} / {r['total']}"]
                        else:
...
def self_test():
    ...
    main_sample = {"mode":"main", "mockup":"C:/测试/主图.psd",
                   "materials":["C:/测试/01.png", "C:/测试/02.jpg"],
                   "outputFolder":"C:/测试/主图输出", "fit":"cover", "naming":{"enabled":False}}
    main_script = build_js(main_sample)
    main_checks = ["processMain", "collectSmartObjects", "smartObjectKey", "materialIndex%materials.length",
