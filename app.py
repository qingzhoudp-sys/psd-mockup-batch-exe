...
(function () {
    ...
    function processOne(artFile, mockupFile, outputFolder) {
        ...
        try {
            ...
            if (CFG.slices) {
                try { exportSlicesJpg(doc, outputFolder, safe(strip(decodeURI(artFile.name))), CFG.quality); }
    ...
    function replaceContents(artFile, fit) {
        ...
        smart.flatten();
        smart.save(); smart.close(SaveOptions.DONOTSAVECHANGES); app.activeDocument=parent;
    }
    ...
    function exportSlicesJpg(doc, outputFolder, artworkBase, jpgQuality) {
        app.activeDocument=doc;
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Checkbutton(opts, text="PSD 存在用户切片时导出 JPG 切片", variable=self.slices).pack(anchor="w", pady=(12, 0))
...
def self_test():
    ...
    slice_checks = ["exportSlicesJpg", "saveUserSlices", 'c("SLUs")', 'c("JPEG")', "smart.flatten()"]
