...
(function () {
    ...
    function processOne(artFile, mockupFile, outputFolder) {
        ...
        try {
            ...
            if (CFG.slices) exportSlicesBoth(doc, outputFolder, safe(strip(decodeURI(artFile.name))), CFG.quality);
        } finally {
    ...
    function exportSlicesBoth(doc, outputFolder, artworkBase, jpgQuality) {
        app.activeDocument=doc;
        ...
        var pngFolder=new Folder(root.fsName+"/PNG");
        if(!jpgFolder.exists && !jpgFolder.create()) throw new Error("无法创建 JPG 切片文件夹");
        if(!pngFolder.exists && !pngFolder.create()) throw new Error("无法创建 PNG 切片文件夹");
        saveUserSlices(jpgFolder,"jpg",jpgQuality);
        saveUserSlices(pngFolder,"png",jpgQuality);
        exported.push(jpgFolder.fsName+" [JPG用户切片]");
        exported.push(pngFolder.fsName+" [PNG用户切片]");
    }
    function saveUserSlices(folder, format, jpgQuality) {
        ...
        if(format=="png") {
            opts.putEnumerated(c("Fmt "),c("IRFm"),c("PN24"));
            opts.putBoolean(c("Intr"),false);
            opts.putBoolean(c("Trns"),true);
        } else {
            opts.putEnumerated(c("Fmt "),c("IRFm"),c("JPEG"));
            opts.putBoolean(c("Intr"),false);
            opts.putInteger(c("Qlty"),Math.max(0,Math.min(100,Math.round(jpgQuality/12*100))));
        }
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Checkbutton(opts, text="同时导出 PSD 用户切片（JPG + PNG）", variable=self.slices).pack(anchor="w", pady=(12, 0))
    ...
    def _start(self):
        ...
        settings = {"mockup":str(Path(self.mockup.get()).resolve()), "inputFolder":str(Path(self.inputs.get()).resolve()),
                    ...
                    "quality":max(1,min(12,self.quality.get()))}
...
def self_test():
    ...
    slice_checks = ["exportSlicesBoth", "saveUserSlices", 'c("SLUs")', 'c("JPEG")', 'c("PN24")']
